...
def createRangeImageWildVPR(points, crop=True, crop_distance=False, distance_threshold=100):
    if crop_distance:
        dis = np.linalg.norm(points[:, :3], axis=1)
        points = points[np.where(dis<distance_threshold)[0]]
    image,_,_,_ = range_projection(points, fov_up=45.0, proj_H=160, max_range=50)
    image = Image.fromarray(np.uint8(image * 5.1) , 'L')
    image = np.stack((image,)*3, axis=-1)
    if crop:
        clipStart = image.shape[1] // 3      
        image = image[:, clipStart:clipStart*2, :]
    return image
